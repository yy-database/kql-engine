...
impl Lowerer {
    ...
    fn lower_type(&mut self, ty: ast::Type) -> Result<HirType> {
        match ty {
            ast::Type::Named(n) => {
                match n.name.as_str() {
                    "Int" | "i32" | "i64" => Ok(HirType::Primitive(PrimitiveType::Int)),
                    "Float" | "f32" | "f64" => Ok(HirType::Primitive(PrimitiveType::Float)),
                    ...
                    _ => {
                        if let Some(&id) = self.db.name_to_id.get(&n.name) {
                            ...
                            Err(KqlError::SemanticError {
                                span: n.span,
                                message: format!("Unknown type: {}", n.name),
                            })
                        }
                    }
                }
    ...
    fn lower_expr(&mut self, expr: ast::Expr) -> Result<HirExpr> {
        match expr {
            ast::Expr::Literal(l) => {
                let (kind, ty) = match l.kind {
                    ast::LiteralKind::Number(n) => (
                        ...
                        HirType::Primitive(PrimitiveType::Int),
                    ),
                    ast::LiteralKind::String(s) => (
                        HirExprKind::Literal(HirLiteral::String(s)),
            ...
            ast::Expr::Binary(b) => {
                ...
                Ok(HirExpr {
                    kind: HirExprKind::Binary { left: Box::new(left), op, right: Box::new(right) },
                    ty,
                    span: b.span,
                })
            }
            ast::Expr::Unary(u) => {
                ...
                Ok(HirExpr {
                    kind: HirExprKind::Unary { op, expr: Box::new(expr) },
                    ty,
                    span: u.span,
                })
            }
            ast::Expr::Call(c) => {
                ...
                Ok(HirExpr {
                    kind: HirExprKind::Call { func: Box::new(func), args },
                    ty,
                    span: c.span,
                })
            }
