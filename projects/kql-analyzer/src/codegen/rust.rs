...
impl RustGenerator {
    ...
    pub fn generate(&self) -> String {
        ...
        out.push_str("use sqlx::{FromRow, Postgres, MySql, Sqlite};\n");
        out.push_str("use kql_runtime::{KqlEntity, KqlRepository, async_trait};\n\n");
    ...
    fn is_json_type(&self, ty: &HirType) -> bool {
        match ty {
            HirType::List(_) => true,
            HirType::Struct(id) => {
                if let Some(s) = self.db.structs.get(id) {
                    matches!(s.layout, Some(crate::hir::StructLayout::Json))
                } else {
                    false
                }
            }
            HirType::Enum(id) => {
                if let Some(e) = self.db.enums.get(id) {
                    ...
                    e.variants.iter().any(|v| v.fields.is_some())
                } else {
                    false
                }
            }
            HirType::Optional(inner) => self.is_json_type(inner),
            _ => false,
        }
    }
    ...
    fn gen_struct(&self, s: &HirStruct, indent: usize) -> String {
        ...
        for f in &s.fields {
            ...
            if is_table {
                match &f.ty {
                    ...
                    _ => {
                        if self.is_json_type(&f.ty) {
                            out.push_str(&format!("{}    #[sqlx(json)]\n", indent_str));
                        }
                    }
                }
            }
        ...
        if is_table {
            ...
            out.push_str(&format!("{}    fn table_name() -> &'static str {{\n", indent_str));
            out.push_str(&format!("{}        \"{}\"\n", indent_str, s.name.to_lowercase()));
            out.push_str(&format!("{}    }}\n", indent_str));
            out.push_str(&format!("{}}}\n", indent_str));
        }
    ...
    fn gen_repository(&self, s: &HirStruct, indent: usize) -> String {
        ...
        let pool_getter = match self.dialect {
            SqlDialect::Postgres => "as_postgres().unwrap()",
            SqlDialect::MySql => "as_mysql().unwrap()",
            SqlDialect::Sqlite => "as_sqlite().unwrap()",
        };

        out.push_str(&format!("{}}}\n\n", indent_str));

        out.push_str(&format!("{}#[async_trait]\n", indent_str));
        out.push_str(&format!("{}impl KqlRepository<{}> for {} {{\n", indent_str, s.name, repo_name));
        ...
        if let Some(pk) = pk_field {
            ...
            // Find method
            out.push_str(&format!("{}    async fn find_by_id(&self, id: {}) -> Result<Option<{}>, sqlx::Error> {{\n", indent_str, pk_ty, s.name));
            ...
            out.push_str(&format!("{}            .fetch_optional(self.pool.{})\n", indent_str, pool_getter));
            out.push_str(&format!("{}            .await\n", indent_str));
            out.push_str(&format!("{}    }}\n\n", indent_str));

            // Delete method
            out.push_str(&format!("{}    async fn delete(&self, id: {}) -> Result<(), sqlx::Error> {{\n", indent_str, pk_ty));
            ...
            // Fallback implementations if no PK
            out.push_str(&format!("{}    async fn find_by_id(&self, _id: ()) -> Result<Option<{}>, sqlx::Error> {{ Ok(None) }}\n", indent_str, s.name));
            out.push_str(&format!("{}    async fn delete(&self, _id: ()) -> Result<(), sqlx::Error> {{ Ok(()) }}\n", indent_str));
        }
        ...
        if !insert_fields.is_empty() {
            ...
            out.push_str(&format!("{}    async fn insert(&self, model: &{}) -> Result<(), sqlx::Error> {{\n", indent_str, s.name));
            ...
            out.push_str(&format!("{}    async fn insert(&self, _model: &{}) -> Result<(), sqlx::Error> {{ Ok(()) }}\n", indent_str, s.name));
        }
        ...
        if let Some(pk) = pk_field {
            ...
            if !update_fields.is_empty() {
                ...
                out.push_str(&format!("{}    async fn update(&self, model: &{}) -> Result<(), sqlx::Error> {{\n", indent_str, s.name));
                ...
                out.push_str(&format!("{}    async fn update(&self, _model: &{}) -> Result<(), sqlx::Error> {{ Ok(()) }}\n", indent_str, s.name));
            }
        } else {
            out.push_str(&format!("{}    async fn update(&self, _model: &{}) -> Result<(), sqlx::Error> {{ Ok(()) }}\n", indent_str, s.name));
        }
        ...
        // List method
        out.push_str(&format!("{}    async fn list(&self) -> Result<Vec<{}>, sqlx::Error> {{\n", indent_str, s.name));
        ...
        out.push_str(&format!("{}    fn query(&self) -> kql_runtime::QueryBuilder<{}> {{\n", indent_str, s.name));
        out.push_str(&format!("{}        kql_runtime::QueryBuilder::new(&self.pool)\n", indent_str));
        out.push_str(&format!("{}    }}\n", indent_str));
    ...
    fn gen_type(&self, ty: &HirType) -> String {
        match ty {
            HirType::Primitive(p) => match p {
                ...
                PrimitiveType::Time => "chrono::NaiveTime".to_string(),
                ...
                PrimitiveType::Json => "serde_json::Value".to_string(),
            },
