...
impl<'a> Parser<'a> {
    ...
    fn parse_variant(&mut self) -> Result<Variant> {
        ...
        Ok(Variant {
            name: name.clone(),
            fields,
            span: Span { start: name.span.start, end: end_pos },
        })
    }
    ...
    pub fn parse_type(&mut self) -> Result<Type> {
        ...
        if self.curr.kind == TokenKind::Question {
            ...
            ty = Type::Optional(OptionalType {
                inner: Box::new(ty),
                span: Span { start, end: q_span.end },
            });
        }
    ...
    fn parse_prefix(&mut self) -> Result<Expr> {
        ...
        match token.kind {
            ...
            TokenKind::String => {
                let expr = Expr::Literal(LiteralExpr {
                    kind: LiteralKind::String(token.text.clone()),
                    span: token.span,
                });
                self.advance();
                Ok(expr)
            ...
            TokenKind::Boolean => {
                let expr = Expr::Literal(LiteralExpr {
                    kind: LiteralKind::Boolean(token.text == "true"),
                    span: token.span,
                });
                self.advance();
                Ok(expr)
    ...
    fn parse_infix(&mut self, left: Expr) -> Result<Expr> {
        ...
        match token.kind {
            ...
            TokenKind::DoubleEq | TokenKind::NotEq | TokenKind::Greater | TokenKind::Less | TokenKind::GreaterEq | TokenKind::LessEq |
